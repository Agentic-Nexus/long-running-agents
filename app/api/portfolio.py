    ...
    try:
        async with session_scope() as session:
            ...
            new_portfolio = await service.create_portfolio(
                user_id=portfolio.user_id or 1,
                name=portfolio.name,
...
async def update_portfolio_value(portfolio_id: int):
    """
    更新投资组合价值
    """
    async with session_scope() as session:
...
async def add_portfolio_history(portfolio_id: int):
    """
    添加投资组合历史快照
    """
    async with session_scope() as session:
...
async def get_portfolio_report(portfolio_id: int):
    """
    获取投资组合分析报告
    """
    async with session_scope() as session:
